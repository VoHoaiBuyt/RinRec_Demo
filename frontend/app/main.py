...
/* ==============================================================================
   GLASSMORPHISM ENTERPRISE LOGIN FORM
   ============================================================================== */
.glass-login-wrapper {
    position: relative;
    max-width: 500px;
...
/* Ambient Floating Glow Orbs behind Glass */
.glass-glow-orb-1 {
    position: absolute;
    width: 240px;
    height: 240px;
    top: -40px;
    left: -50px;
    background: radial-gradient(circle, rgba(0, 177, 79, 0.45) 0%, rgba(0, 177, 79, 0) 70%);
    border-radius: 50%;
    filter: blur(40px);
    z-index: 0;
    pointer-events: none;
...
.glass-login-card {
    position: relative;
    z-index: 1;
    background: linear-gradient(135deg, rgba(15, 23, 42, 0.8) 0%, rgba(10, 37, 64, 0.88) 100%);
    ...
    box-shadow: 0 25px 60px -15px rgba(0, 0, 0, 0.6), 
                ...
                0 10px 30px -10px rgba(0, 177, 79, 0.25);
    color: #FFFFFF;
}
...
.glass-login-title {
    font-size: 1.75rem;
    font-weight: 800;
    background: linear-gradient(135deg, #FFFFFF 0%, #E2E8F0 60%, #34D399 100%);
...
/* Custom form styling for Glassmorphism Login */
[data-testid="stForm"]:has(input[type="password"]) {
    position: relative !important;
    z-index: 1 !important;
    background: linear-gradient(135deg, rgba(15, 23, 42, 0.82) 0%, rgba(10, 37, 64, 0.9) 100%) !important;
    ...
    box-shadow: 0 25px 60px -15px rgba(0, 0, 0, 0.6), 
                ...
                0 10px 30px -10px rgba(0, 177, 79, 0.25) !important;
}

[data-testid="stForm"]:has(input[type="password"]) .stTextInput label {
    color: #E2E8F0 !important;
    font-weight: 600 !important;
    font-size: 0.88rem !important;
}

[data-testid="stForm"]:has(input[type="password"]) .stTextInput input {
...
[data-testid="stForm"]:has(input[type="password"]) .stTextInput input:focus {
    border-color: #00B14F !important;
    box-shadow: 0 0 0 3px rgba(0, 177, 79, 0.3) !important;
    background: rgba(15, 23, 42, 0.92) !important;
}

[data-testid="stForm"]:has(input[type="password"]) button[type="submit"] {
    ...
    border-radius: 12px !important;
    box-shadow: 0 10px 25px -5px rgba(0, 177, 79, 0.45) !important;
    transition: all 0.25s ease !important;
    margin-top: 0.8rem !important;
}

[data-testid="stForm"]:has(input[type="password"]) button[type="submit"]:hover {
    transform: translateY(-2px) !important;
    box-shadow: 0 15px 30px -5px rgba(0, 177, 79, 0.6) !important;
    background: linear-gradient(135deg, #02c95b 0%, #059669 100%) !important;
}
.user-sidebar-card {
...
# ------------------------------------------------------------------------------
# MÀN HÌNH ĐĂNG NHẬP & ĐĂNG KÝ HỆ THỐNG (GLASSMORPHISM PURPLE THEME)
# ------------------------------------------------------------------------------
if st.session_state.authenticated_user is None:
    if render_auth_screen is not None:
        render_auth_screen(authenticate_user, register_user)
    else:
        col_l1, col_l2, col_l3 = st.columns([1, 2, 1])
        with col_l2:
            st.title("Welcome Back")
            in_u = st.text_input("Username or Email")
            in_p = st.text_input("Password", type="password")
            if st.button("Sign In", use_container_width=True):
                res = authenticate_user(in_u, in_p)
                if res.get("success"):
...
with st.sidebar:
    ...
    with col_sb_act1:
        if st.button("➕ Thêm KH Mới", key="btn_sb_new_cust", use_container_width=True):
            st.session_state.selected_cif = None
            if "cif" in st.query_params:
                del st.query_params["cif"]
            st.rerun()
    with col_sb_act2:
        if st.button("🔄 Làm Mới Data", key="btn_sb_refresh", use_container_width=True):
            load_app_data.clear()
            st.toast("Đã làm mới dữ liệu từ MongoDB Atlas!")
            st.rerun()
...
with tab1:
    ...
    with st.expander("🛡️ **QUẦY GIAO DỊCH SỐ: NHẬN DIỆN KHUÔN MẶT KHÁCH HÀNG (AI Smart Counter & Mobile eKYC)**", expanded=(selected_cif is None)):
        ekyc_tab1, ekyc_tab2, ekyc_tab3, ekyc_tab4 = st.tabs([
            "📱 Xác Thực Qua Di Động (Cross-Device QR)",
            "📷 Quét Khuôn Mặt Tại Quầy (Webcam)",
            "📁 Tải Ảnh Đối Soát Sinh Trắc Học",
            "➕ Mở Mã CIF & Đăng Ký Khách Hàng Mới"
        ])
        ...
        # TAB D: MỞ MÃ CIF & ĐĂNG KÝ KHÁCH HÀNG MỚI (ONBOARDING)
        with ekyc_tab4:
            st.markdown("##### ➕ Mở Mã CIF & Đăng Ký Hồ Sơ Khách Hàng Mới (MongoDB Atlas)")
            st.caption("Khởi tạo hồ sơ khách hàng 360°, định danh sinh trắc học và phân bổ hạn mức ban đầu:")
            
            with st.form("form_create_new_customer"):
                f_c1, f_c2 = st.columns(2, gap="large")
                with f_c1:
                    new_c_name = st.text_input("Họ và Tên khách hàng (*):", placeholder="vd: Nguyễn Văn An...")
                    new_c_id = st.text_input("Số CMND / Căn cước CCCD (*):", placeholder="vd: 001202008899...")
                    new_c_phone = st.text_input("Số điện thoại liên hệ (*):", placeholder="vd: 0988123456...")
                    new_c_email = st.text_input("Địa chỉ Email:", placeholder="vd: nguyenvanan@gmail.com...")
                    new_c_addr = st.text_input("Địa chỉ thường trú / Liên hệ:", value="TP. Hà Nội")
                with f_c2:
                    new_c_seg = st.selectbox(
                        "Phân khúc khách hàng mục tiêu (*):",
                        ["MASS", "PRIME", "DIAMOND"],
                        format_func=lambda s: "💎 DIAMOND VIP (Khách hàng ưu tiên cao cấp)" if s == "DIAMOND" else ("⭐ PRIME PRIORITY (Khách hàng ưu tiên)" if s == "PRIME" else "🔷 MASS (Khách hàng phổ thông)")
                    )
                    new_c_casa = st.number_input("Số dư tài khoản ban đầu (CASA - VND):", min_value=0, value=2000000, step=1000000, format="%d")
                    new_c_job = st.text_input("Nghề nghiệp / Lĩnh vực công tác:", value="Kinh doanh tự do / Văn phòng")
                    new_c_img = st.file_uploader("Ảnh chân dung đăng ký sinh trắc học (eKYC):", type=["jpg", "jpeg", "png"], key="new_c_face_img")
                    
                submit_create_cust = st.form_submit_button("💾 Khởi Tạo Hồ Sơ & Mở Mã CIF", use_container_width=True)
                if submit_create_cust:
                    if not new_c_name.strip():
                        st.error("Vui lòng nhập Họ và Tên khách hàng!")
                    elif not new_c_id.strip():
                        st.error("Vui lòng nhập số CMND / CCCD!")
                    else:
                        with st.spinner("Đang khởi tạo mã CIF và đồng bộ lên MongoDB Atlas..."):
                            if create_customer:
                                res_nc = create_customer({
                                    "full_name": new_c_name,
                                    "id_number": new_c_id,
                                    "phone_number": new_c_phone,
                                    "email": new_c_email,
                                    "address": new_c_addr,
                                    "segment": new_c_seg,
                                    "casa_balance": new_c_casa,
                                    "occupation": new_c_job,
                                    "branch_code": "CN001"
                                })
                                if res_nc.get("success"):
                                    created_cif = res_nc.get("cif")
                                    # Nếu có ảnh tải lên, enroll vào face_engine
                                    if new_c_img is not None and face_engine:
                                        try:
                                            face_engine.enroll_customer_face(created_cif, new_c_name, new_c_seg, new_c_img.getvalue())
                                        except Exception:
                                            pass
                                    st.session_state.selected_cif = created_cif
                                    st.session_state.ekyc_verified = True
                                    st.session_state.ekyc_info = {
                                        "is_identified": True,
                                        "cif_number": created_cif,
                                        "customer_name": new_c_name,
                                        "segment": new_c_seg,
                                        "confidence": 100.0,
                                        "engine": "MongoDB Atlas eKYC Biometrics"
                                    }
                                    st.query_params["cif"] = created_cif
                                    st.success(f"✅ {res_nc.get('message')}")
                                    st.toast(f"Đã mở mã CIF {created_cif} thành công!")
                                    load_app_data.clear()
                                    time.sleep(0.6)
                                    st.rerun()
                                else:
                                    st.error(res_nc.get("message"))
    ...
    if selected_name and selected_cif:
        ...
        with col_history:
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            with st.expander("➕ **Ghi Nhận Giao Dịch Mới Tại Quầy (Real-Time Ingestion)**", expanded=False):
                st.caption(f"Ghi nhận giao dịch phát sinh trực tiếp cho khách hàng **{selected_name}** ({selected_cif}):")
                with st.form(f"form_add_txn_{selected_cif}"):
                    c_t1, c_t2 = st.columns(2)
                    with c_t1:
                        common_services = [
                            ("Nộp tiền mặt vào tài khoản thanh toán CASA", "B. Giao dịch tiền mặt"),
                            ("Gửi tiết kiệm có kỳ hạn tích lũy", "C. Tiết kiệm & Tiền gửi"),
                            ("Rút tiền mặt tại quầy giao dịch", "B. Giao dịch tiền mặt"),
                            ("Chuyển tiền quốc tế & Thanh toán ngoại tệ", "F. Ngoại tệ"),
                            ("Đăng ký phát hành thẻ tín dụng quốc tế", "D. Thẻ"),
                            ("Đăng ký vay vốn kinh doanh / tiêu dùng", "G. Tín dụng"),
                            ("Tham gia hợp đồng bảo hiểm nhân thọ", "H. Bảo hiểm & Đầu tư"),
                            ("Kích hoạt tiện ích ngân hàng số VPBank NEO", "I. Ngân hàng số & Hỗ trợ"),
                            ("Thanh toán hóa đơn điện / nước / dịch vụ", "E. Chuyển tiền & Thanh toán")
                        ]
                        sel_srv_idx = st.selectbox(
                            "Chọn nghiệp vụ / dịch vụ thực hiện:",
                            range(len(common_services)),
                            format_func=lambda i: common_services[i][0]
                        )
                        srv_title, srv_group = common_services[sel_srv_idx]
                        txn_channel = st.selectbox("Kênh giao dịch:", ["QUẦY", "VPBank NEO", "ATM/POS"], index=0)
                    with c_t2:
                        txn_amount = st.number_input("Số tiền giao dịch (VND):", min_value=0, step=1000000, value=5000000, format="%d")
                        txn_notes = st.text_input("Ghi chú nghiệp vụ / Mã bút toán:", placeholder="vd: Nộp tiền mặt tại PGD...")
                        
                    submit_txn = st.form_submit_button("💾 Xác Nhận & Đồng Bộ Vào MongoDB", use_container_width=True)
                    if submit_txn:
                        if add_transaction:
                            with st.spinner("Đang lưu giao dịch vào MongoDB Atlas..."):
                                res_tx = add_transaction({
                                    "cif_number": selected_cif,
                                    "customer_name": selected_name,
                                    "segment": segment,
                                    "service_name": srv_title,
                                ...
                                if res_tx.get("success"):
                                    st.success(res_tx.get("message"))
                                    st.toast("✅ Giao dịch mới đã được lưu và cập nhật ngay lập tức!")
                                    load_app_data.clear()
                                    time.sleep(0.5)
                                    st.rerun()
                                else:
                                    st.error(res_tx.get("message"))
...
"""
                    ...
                    # Nút tương tác thông minh cho từng sản phẩm
                    btn_col1, btn_col2, btn_col3 = st.columns([1.4, 1.1, 1])
                    with btn_col1:
                        with st.popover(f"📝 Chốt Đơn / Ghi Nhận #{rank}", use_container_width=True):
                            st.markdown(f"**Ghi nhận tư vấn: {p_name}**")
                            with st.form(f"form_log_deal_{rank}_{selected_cif}"):
                                deal_status = st.selectbox(
                                    "Kết quả tư vấn:",
                                    [
                                        "✅ CHỐT HỢP ĐỒNG THÀNH CÔNG",
                                        "⏳ KHÁCH QUAN TÂM / CÂN NHẮC",
                                        "❌ KHÁCH HÀNG TỪ CHỐI"
                                    ],
                                    key=f"deal_st_{rank}_{selected_cif}"
                                )
                                deal_val = st.text_input("Hạn mức / Giá trị hợp đồng:", value=p_price, key=f"deal_val_{rank}_{selected_cif}")
                                deal_note = st.text_input("Ghi chú tư vấn của GDV:", placeholder="vd: Khách đồng ý mở thẻ hạn mức 50tr...", key=f"deal_nt_{rank}_{selected_cif}")
                                submit_deal = st.form_submit_button("💾 Lưu Vào Sổ Nhật Ký", use_container_width=True)
                                if submit_deal:
                                    if log_consultation:
                                        res_log = log_consultation({
                                            "cif_number": selected_cif,
                                            "customer_name": selected_name,
                                            "product_name": p_name,
                                            "category": p_group,
                                            "status": deal_status,
                                            "deal_amount": deal_val,
                                            "notes": deal_note,
                                            "teller_name": user_fullname,
                                            "teller_code": user_code
                                        })
                                        if res_log.get("success"):
                                            st.toast(f"✅ Đã ghi nhận: {deal_status} cho sản phẩm {p_name}!")
                                            time.sleep(0.4)
                                            st.rerun()
                    with btn_col2:
                        if st.button(f"📱 Gửi App #{rank}", key=f"btn_send_{rank}_{selected_cif}", use_container_width=True):
                            st.toast(f"Đã gửi ưu đãi [{p_name}] tới VPBank NEO của {selected_name}!")
                    with btn_col3:
                        if st.button(f"📑 Mở Hồ Sơ #{rank}", key=f"btn_open_{rank}_{selected_cif}", use_container_width=True):
                            st.toast(f"Đã mở quy trình thẩm định cho [{p_name}]!")
                    st.markdown("<div style='margin-bottom: 0.8rem;'></div>", unsafe_allow_html=True)
            else:
                st.info("Chưa có danh mục sản phẩm đề xuất sẵn cho khách hàng này.")

            # 4. Bảng Nhật Ký Tư Vấn & Chốt Hợp Đồng Của Khách Hàng
            st.markdown("<br>", unsafe_allow_html=True)
            if get_consultation_logs:
                c_logs = get_consultation_logs(selected_cif)
                if c_logs:
                    df_logs = pd.DataFrame(c_logs)
                    cols_log_show = [c for c in ['created_at', 'product_name', 'status', 'deal_amount', 'teller_name', 'notes'] if c in df_logs.columns]
                    rename_log = {
                        'created_at': 'Thời Gian',
                        'product_name': 'Sản Phẩm Đề Xuất',
                        'status': 'Kết Quả Tư Vấn',
                        'deal_amount': 'Hạn Mức / Giá Trị',
                        'teller_name': 'Giao Dịch Viên',
                        'notes': 'Ghi Chú GDV'
                    }
                    render_banking_table(
                        "NHẬT KÝ TƯ VẤN & KẾT QUẢ BÁN CHÉO (MONGODB ATLAS)",
                        df_logs[cols_log_show].rename(columns=rename_log),
                        max_height=320,
                        badge_text=f"{len(df_logs)} Lượt tư vấn"
                    )
                else:
                    st.info("💡 **Chưa có nhật ký tư vấn nào cho khách hàng này.** Bấm nút *'📝 Chốt Đơn / Ghi Nhận'* ở thẻ sản phẩm bên trên để lưu kết quả tư vấn vào MongoDB Atlas!")
...
with tab3:
    ...
    # Khu vực Thêm Sản Phẩm Mới Vào Danh Mục
    with st.expander("➕ **Thêm Sản Phẩm / Gói Dịch Vụ Mới Vào Danh Mục (MongoDB Atlas)**", expanded=False):
        st.caption("Cập nhật danh mục sản phẩm ngân hàng, hạn mức an toàn và biểu phí ưu đãi trực tiếp vào CSDL:")
        with st.form("form_add_new_product"):
            p_c1, p_c2 = st.columns(2, gap="large")
            with p_c1:
                new_p_name = st.text_input("Tên sản phẩm tài chính (*):", placeholder="vd: Tiết Kiệm Tích Lũy Linh Hoạt Flexi...")
                new_p_code = st.text_input("Mã sản phẩm (Bỏ trống để tự động sinh):", placeholder="vd: SP026...")
                new_p_group = st.selectbox("Nhóm sản phẩm:", ["Tiết kiệm", "Thẻ", "Vay", "Bảo hiểm", "Đầu tư", "Ngoại tệ", "Dịch vụ số", "Doanh nghiệp", "Phân khúc"])
                new_p_val = st.text_area("Đề xuất giá trị / Giá trị cốt lõi mang lại cho KH:", placeholder="Mô tả quyền lợi nổi bật giúp GDV dễ dàng tư vấn...")
            with p_c2:
                new_p_seg = st.selectbox("Phân khúc áp dụng:", ["ALL", "MASS", "PRIME", "DIAMOND"], index=0)
                new_p_min = st.number_input("Hạn mức tối thiểu / Số tiền tham gia (VND):", min_value=0, value=10000000, step=5000000, format="%d")
                new_p_fee = st.text_input("Lãi suất / Biểu phí ưu đãi:", value="Lãi suất cạnh tranh theo kỳ hạn")
                new_p_prio = st.slider("Độ ưu tiên hiển thị (1 - 10):", min_value=1, max_value=10, value=7)
                
            submit_new_prod = st.form_submit_button("💾 Thêm Vào Danh Mục Sản Phẩm (MongoDB)", use_container_width=True)
            if submit_new_prod:
                if not new_p_name.strip():
                    st.error("Vui lòng nhập tên sản phẩm tài chính!")
                else:
                    if add_product:
                        with st.spinner("Đang lưu sản phẩm mới vào MongoDB Atlas..."):
                            res_np = add_product({
                                "Ma_SP": new_p_code,
                                "Ten_san_pham": new_p_name,
                                "Nhom": new_p_group,
                                "Gia_tri_cot_loi": new_p_val,
                                "Phan_khuc": new_p_seg,
                                "So_tien_toi_thieu": new_p_min,
                                "Lai_suat_Phi": new_p_fee,
                                "Uu_tien": new_p_prio
                            })
                            if res_np.get("success"):
                                st.success(res_np.get("message"))
                                st.toast(f"✅ Đã thêm sản phẩm {new_p_name} vào danh mục!")
                                load_app_data.clear()
                                time.sleep(0.5)
                                st.rerun()
                            else:
                                st.error(res_np.get("message"))
    st.markdown("<br>", unsafe_allow_html=True)
